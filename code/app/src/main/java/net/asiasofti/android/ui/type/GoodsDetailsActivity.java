...
public class GoodsDetailsActivity extends Activity {
	...
	public void loadingGoodsDetailsData(String s) {
		String url = (new StringBuilder(
				Constants.URL_GOODSDETAILS + "&goods_id="))
				.append(s).toString();
						...
						new net.asiasofti.android.handler.RemoteDataHandler.Callback() {
							...
							public void dataLoaded(ResponseData responsedata) {
								...
								try {
									...
									String goodsImage = respJson.getString("goods_image");
									String mansongInfo = respJson.getString("mansong_info");
									String goodsCommondList = respJson.getString("goods_commend_list");
									final String specListStr = respJson.getString("spec_list");
									goodsdetails = GoodsDetails.newInstanceList(goodsInfo);
									textGoodsDetailsName.setText(goodsdetails.getGoods_name());
									textGoodsDetailsPrice.setText((new StringBuilder("￥")).append(goodsdetails.getGoods_price()).toString());
									textGoodsDetailsMarketPrice.setText((new StringBuilder("￥")).append(goodsdetails.getGoods_marketprice()).toString());
									textGoodsDetailsMarketPrice.getPaint().setFlags(16);
									textGoodsDetailsStorage.setText((new StringBuilder(String.valueOf(goodsdetails.getGoods_storage()))).append("件").toString());
									goods_kucun = Integer.parseInt(goodsdetails.getGoods_storage());
									GoodsDetailsActivity.this.adapter.setGoods_image(goodsImage.split(","));
									GoodsDetailsActivity.this.adapter.notifyDataSetChanged();
									if (goodsdetails.getPromotion_type().equals("groupbuy"))
									{
										buttonAddShopCart.setVisibility(8);
									}
									if (goodsdetails.getPromotion_price() != null
										&& !goodsdetails.getPromotion_price().equals("")
										&& !goodsdetails.getPromotion_price().equals("null"))
									{
										textGoodsDetailsPrice.setText((new StringBuilder("￥")).append(goodsdetails.getPromotion_price()).toString());
									}
									if (goodsdetails.getUpper_limit() != null
										&& !goodsdetails.getUpper_limit().equals("")
										&& !goodsdetails.getUpper_limit().equals("null"))
									{
										upper_limit = Integer.parseInt(goodsdetails.getUpper_limit());
									}
									if (!mansongInfo.equals("") && !mansongInfo.equals("null")&& mansongInfo != null) {
										...
										ArrayList mansongRuleList = ManSongRules
												.newInstanceList(ManSongInFo
														...
														.getRules());
										manSongRulesAdapter
												.setmRules(mansongRuleList);
										manSongRulesAdapter
												.notifyDataSetChanged();
									}

									
									ArrayList commendList = CommendList.newInstanceList(goodsCommondList);
									...
									Iterator currentGoodsSpecIt = currentGoodsSpecJson.keys();
									...
									while (currentGoodsSpecIt.hasNext()) {
										...
										String currentGoodsSpecId = currentGoodsSpecIt.next().toString();
										String currentGoodsSpecVaue = currentGoodsSpecJson.getString(currentGoodsSpecId);
										oldspec[i] = Integer.parseInt(currentGoodsSpecId);
										i++;
									...
									while (specNameIt.hasNext()) 
									{
										...
										String keyName = specNameIt.next().toString();//example{1:"颜色"，2:尺码} 则 keyname=1 keyvalue="颜色"
										keyValue = specNameJson.getString(keyName);
										String valueStr = specValueJson.getString(keyName);//spec value{"464":"金驼","465":"翠绿"}
										JSONObject valueStrJson = new JSONObject(
												valueStr);
										Iterator iterator2 = valueStrJson.keys();
										while (iterator2.hasNext()) {
											String specId = iterator2.next().toString();
											String specName = valueStrJson.getString(specId);
											Spec spec = new Spec();
										...
										TextView specNameText = (TextView) goodsDetailSpecview.findViewById(R.id.textSpecName);
										final MyGridView gridViewSpec = (MyGridView) goodsDetailSpecview.findViewById(R.id.gridViewSpec);
										final SpecGridViewAdapter adapter = new SpecGridViewAdapter(GoodsDetailsActivity.this);
										...
										linearLayoutSpecView.addView(goodsDetailSpecview);
										gridViewSpec.setOnItemClickListener(new android.widget.AdapterView.OnItemClickListener() {
													public void onItemClick(
														...
														String oldIdStr = "";
														String newIdStr = "";
														Spec selectSpec = (Spec) gridViewSpec.getItemAtPosition(position);//当前商品的规格
														Arrays.sort(oldspec);
														...
														for (int j = 0; j <= oldspec.length - 1; j++) 
														{
															if (oldspec[j] != 0) 
															{
																...
																String valueTemp = String.valueOf(oldspec[j]).replace(adapter.getOld_specID(), selectSpec.getSpecID());
																		
																oldspec[j] = Integer.parseInt(
																		(String.valueOf(oldspec[j]).replace(
																						adapter.getOld_specID(),
																						selectSpec.getSpecID())));
																newIdStr = (new StringBuilder(String.valueOf(newIdStr))).append(oldspec[j]).append("|").toString();
															}

														}
														int[] temp2 = oldspec;
														
														String subNewIdStr = newIdStr.substring(0,
																-1 + newIdStr.length());
														

														String subOldIdStr = oldIdStr.substring(0,
																-1 + oldIdStr.length());

														if (subNewIdStr.equals(subOldIdStr)) {
															return;
														}

														try {
															JSONObject specListJson = new JSONObject(specListStr);//specListStr 属性列表对象中寻找组合 ID 
															Iterator specListJsonIt = specListJson.keys();//格式"465|466":"402" 每种属性组合对应一个商品ID

															while (specListJsonIt.hasNext()) {
																String commonSpecValue = specListJsonIt.next().toString();//commonSpecValue 属性ID 组合
																System.out.println((new StringBuilder(
																				...
																				.append(subOldIdStr)
																				.toString());
																if (commonSpecValue.equals(subNewIdStr)) {
																	String commonSpecID = specListJson.getString(commonSpecValue); //commonSpecID 组合对应的最终goodsID 
																	System.out.println((new StringBuilder(
																					"跳转的GOODSID-->"))
																					.append(commonSpecID)
																					.toString());
																	Intent intent = new Intent(GoodsDetailsActivity.this,net.asiasofti.android.ui.type.GoodsDetailsActivity.class);
																	intent.putExtra("goods_id",commonSpecID);
