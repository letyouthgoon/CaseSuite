...
public class MyApp extends Application
{
	...
	{
		{
			if ("mounted".equals(Environment.getExternalStorageState()))
			{
				...
				if (file.mkdirs())
					System.out.println((new StringBuilder("SD锟斤拷锟斤拷锟斤拷目录:")).append(file.getAbsolutePath()).append("锟窖达拷锟斤拷!").toString());
				else
				...
				if (!file1.exists())
				{
					if (file1.mkdirs())
					{
						System.out.println((new StringBuilder("SD锟斤拷锟斤拷片锟斤拷锟斤拷目录:")).append(file1.getAbsolutePath()).append("锟窖达拷锟斤拷!").toString());
						return;
					} else
					{
	...
	{
		IsCheckLogin = flag;
		sysInitSharedPreferences.edit().putBoolean("IsCheckLogin", IsCheckLogin).commit();
	}

	public void setLoginKey(String s)
	{
		loginKey = s;
		sysInitSharedPreferences.edit().putString("loginKey", loginKey).commit();
	}

	public void setLoginName(String s)
	{
		loginName = s;
		sysInitSharedPreferences.edit().putString("loginName", loginName).commit();
	}
