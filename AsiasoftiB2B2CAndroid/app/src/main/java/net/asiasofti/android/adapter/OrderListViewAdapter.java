...
public class OrderListViewAdapter extends BaseAdapter {
	...
	public View getView(int i, View view, ViewGroup viewgroup) {
		...
		if (view == null) {
			...
			holder.goodsListView = (MyListView) view
					.findViewById(R.id.goodsListView);
			holder.textOrderStoreName = (TextView) view
					.findViewById(R.id.textOrderStoreName);
			holder.textOrderSN = (TextView) view.findViewById(R.id.textOrderSN);
			holder.textOrderAllPrice = (TextView) view
					.findViewById(R.id.textOrderAllPrice);
			holder.textOrderShippingFee = (TextView) view
					.findViewById(R.id.textOrderShippingFee);
			holder.textOrderOperation = (TextView) view
					.findViewById(R.id.textOrderOperation);
			holder.textOrderSuccess = (TextView) view
					.findViewById(R.id.textOrderSuccess);
			view.setTag(holder);
		} else {
			holder = (ViewHolder) view.getTag();
		}
		bean = (OrderList) orderLists.get(i);
		holder.textOrderStoreName.setText((new StringBuilder("店铺名称：")).append(
				bean.getStore_name()).toString());
		holder.textOrderSN.setText((new StringBuilder("订单编号：")).append(
				bean.getOrder_sn()).toString());
		holder.textOrderAllPrice.setText((new StringBuilder("￥")).append(
				bean.getOrder_amount()).toString());
		holder.textOrderShippingFee.setText((new StringBuilder("￥")).append(
				bean.getShipping_fee()).toString());
		arraylist = OrderGoodsList
				.newInstanceList(bean.getExtend_order_goods());
		if (bean.getIf_cancel().equals("true")) {
			holder.textOrderOperation.setText(Html
					.fromHtml("<a href='#'>取消</a>"));
			System.out.println((new StringBuilder(
					"holder.textOrderOperation-->")).append(
					holder.textOrderOperation.getText().toString()).toString());
		} else if (bean.getIf_deliver().equals("true")) {
			holder.textOrderOperation.setText(Html
					.fromHtml("<a href='#'>确认收货</a>"));
		} else if (bean.getIf_lock().equals("true"))
			holder.textOrderOperation.setText(Html
					.fromHtml("<a href='#'>锁定中</a>"));
		else if (bean.getIf_receive().equals("true"))
			holder.textOrderOperation.setText(Html
					.fromHtml("<a href='#'>查看物流</a>"));
		else
	...
	public void loadingSaveOrderData(String paramString1, String paramString2) {
		...
		RemoteDataHandler.asyncPost2(paramString1, localHashMap,
				new RemoteDataHandler.Callback() {
					public void dataLoaded(
						...
						if (paramAnonymousResponseData.getCode() == 200) {
							...
							if (str1.equals("1")) {
								Toast.makeText(
										OrderListViewAdapter.this.context,
										"操作成功", 0).show();
						...
						Toast.makeText(OrderListViewAdapter.this.context,
								"数据加载失败，请稍后重试", 0).show();
					}
				});
	}
