...
public class Type {
	...
	public static ArrayList newInstanceList(String s) {
		...
		try {
			...
			for (int i = 0; i <= jsonarray.length() - 1; i++) {
				...
				arraylist.add(new Type(jsonobject.optString("gc_id"),
						jsonobject.optString("gc_name"), jsonobject
								.optString("image"), jsonobject
								.optString("text")));
			}
	...
	public String toString() {
		return (new StringBuilder("Type [gc_id=")).append(gc_id)
				.append(", gc_name=").append(gc_name).append(", image=")
				.append(image).append(", text=").append(text).append("]")
				.toString();
	}
}
