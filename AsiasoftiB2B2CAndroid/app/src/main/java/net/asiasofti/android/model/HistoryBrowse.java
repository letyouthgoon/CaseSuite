...
public class HistoryBrowse extends Model {
	...
	public static boolean HistortBrowseMoreFlag(int i, int j) {
		return (new Select())
				.from(net.asiasofti.android.model.HistoryBrowse.class)
				.execute().size()
				- i * j > 0;
	}

	public static void deleteAll() {
		(new Delete()).from(net.asiasofti.android.model.HistoryBrowse.class)
				.execute();
	}

	public static void deleteID(String s) {
		(new Delete()).from(net.asiasofti.android.model.HistoryBrowse.class)
				.where("goodsID = ?", new Object[] { s }).execute();
	}
	...
	public static void historyBrowseSava2(GoodsList goodslist) {
		...
		if (from.where("goodsID = ?", aobj).execute().size() <= 0) {
			ArrayList arraylist = (ArrayList) (new Select()).from(
					net.asiasofti.android.model.HistoryBrowse.class).execute();
	...
	public static List searchQueryList(int i, int j) {
		...
		return (new Select())
				.from(net.asiasofti.android.model.HistoryBrowse.class)
				.limit((new StringBuilder(String.valueOf(k))).append(",")
						.append(j).toString()).execute();
	}

	public String toString() {
		return (new StringBuilder("HistoryBrowse [goodsID=")).append(goodsID)
				.append(", imageURL=").append(imageURL).append(", goodsName=")
				.append(goodsName).append(", goodsPrice=").append(goodsPrice)
				.append("]").toString();
	}
}
