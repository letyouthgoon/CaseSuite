...
public class TypeNextActivity extends Activity {
	...
	public void loadingTypeNextCData(final String gc_id) {
		RemoteDataHandler.asyncStringGet(
				(new StringBuilder(Constants.URL_GOODSCLASS + "&gc_id="))
				...
				new net.asiasofti.android.handler.RemoteDataHandler.Callback() {
					...
					public void dataLoaded(ResponseData responsedata) {
						...
						try {
							...
							arraylist.addAll(TypeNext.newInstanceList(s1));
	...
	public void loadingTypeNextGData(String s) {
		RemoteDataHandler.asyncStringGet((new StringBuilder(
				Constants.URL_GOODSCLASS + "&gc_id=")).append(s).toString(),
				new net.asiasofti.android.handler.RemoteDataHandler.Callback() {
	...
	protected void onCreate(Bundle bundle) {
		...
		try {
			String s2 = (new JSONObject(
					(new JSONObject(HttpHelper.get((new StringBuilder(
							Constants.URL_GOODSCLASS + "&gc_id=")).append(s)
							.toString()))).getString("datas")))
					.getString("class_list");
