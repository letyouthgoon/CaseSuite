...
public class BuyStep1Activity extends Activity {
	...
	public void CheackPassword(String s) {
		...
		RemoteDataHandler.asyncPost2(Constants.URL_CHECK_PASSWORD, hashmap,
				new net.asiasofti.android.handler.RemoteDataHandler.Callback() {
					...
					public void dataLoaded(ResponseData responsedata) {
						...
						if (!s1.equals("1")) {
							...
							try {
								String s2 = (new JSONObject(s1))
										.getString("error");
								if (s2 != null)
									Toast.makeText(BuyStep1Activity.this, s2, 0)
											.show();
							...
							Toast.makeText(BuyStep1Activity.this, "验证成功", 0)
									.show();
							PasswordFlag = "通过验证";
						...
						textCheckPasswordFlag.setText((new StringBuilder("("))
								.append(PasswordFlag).append(")").toString());
						return;
...
//		hashmap.put("pay_name", pay_name);
		...
		System.out.println((new StringBuilder("购买参数:")).append(cart_id)
				...
				.toString());
		RemoteDataHandler.asyncPost2(Constants.URL_BUY_STEP2, hashmap,
				new net.asiasofti.android.handler.RemoteDataHandler.Callback() {
	...
	public void loadingBuyStep1Data() {
		...
		RemoteDataHandler.asyncPost2(Constants.URL_BUY_STEP1, hashmap,
				new net.asiasofti.android.handler.RemoteDataHandler.Callback() {
					...
					public void dataLoaded(ResponseData responsedata) {
						...
						if (s.contains("error")) {
							try {
								String s1 = (new JSONObject(s))
										.getString("error");
								if (s1 != null) {
									Toast.makeText(BuyStep1Activity.this, s1, 0)
											.show();
								}
								finish();
						...
						System.out.println((new StringBuilder("json111-->"))
								.append(s).toString());
						...
						AddressDetails addressdetails = AddressDetails
								.newInstanceList(buystep1.getAddress_info());
						...
						if (invoiceinfo != null) {
							...
							textInvInfo.setText(invoiceinfo.getContent());
						} else {
							textInvInfo.setText("不使用发票");
						}
						if (!buystep1.getAvailable_predeposit().equals("0")
								&& !buystep1.getAvailable_predeposit().equals(
										"null")
								&& buystep1.getAvailable_predeposit() != null) {
							linearLayoutAvailablePredeposit1.setVisibility(0);
							textviewAvailablePredeposit.setText(buystep1
									.getAvailable_predeposit());
						} else {
							linearLayoutAvailablePredeposit1.setVisibility(8);
						}
						try {
							JSONObject jsonobject = new JSONObject(buystep1
									.getStore_cart_list());
							...
							if (addressdetails != null) {
								addressId = addressdetails.getAddress_id();
								updataAddress(addressdetails.getCity_id(),
	...
	public void loadingInvoiceContextData() {
		HashMap hashmap = new HashMap();
		hashmap.put("key", myApp.getLoginKey());
		RemoteDataHandler.asyncPost2(Constants.URL_INVOICE_CONTEXT_LIST,
				hashmap,
				new net.asiasofti.android.handler.RemoteDataHandler.Callback() {
					...
					public void dataLoaded(ResponseData responsedata) {
						if (responsedata.getCode() == 200) {
							String s = responsedata.getJson();
							try {
								ArrayList arraylist = InvoiceContextList.newInstanceList((new JSONObject(
										s)).getString("invoice_content_list"));
	...
	public void loadingInvoiceListData() {
		HashMap hashmap = new HashMap();
		hashmap.put("key", myApp.getLoginKey());
		RemoteDataHandler.asyncPost2(Constants.URL_INVOICE_LIST, hashmap,
				new net.asiasofti.android.handler.RemoteDataHandler.Callback() {
					...
					public void dataLoaded(ResponseData responsedata) {
						if (responsedata.getCode() == 200) {
							...
							try {
								...
								invoiceListViewAdapter.setDatas(arraylist);
	...
	public void loadingSaveInvoiceData(String s, String s1, String s2) {
		...
		RemoteDataHandler.asyncPost2(Constants.URL_INVOICE_ADD, hashmap,
				new net.asiasofti.android.handler.RemoteDataHandler.Callback() {
	...
	protected void onCreate(Bundle bundle) {
		...
		System.out
				.println("cartFlag:" + getIntent().getStringExtra("cartFlag"));
				...
				.setOnClickListener(new android.view.View.OnClickListener() {
					...
					public void onClick(View view) {
						...
						if (invoiceListViewAdapter.jilu == -1) {
							String s2 = textInvoiceTitle.getText().toString();// 发票抬头
							String s3 = editUnitName.getText().toString();// 单位名称
							String s4 = textInvoiceContent.getText().toString();// 发票内容
							if (s2.equals("个人")) {
					...
					public void onClick(View view) {
						...
						listview.setOnItemClickListener(new android.widget.AdapterView.OnItemClickListener() {
							...
							public void onItemClick(AdapterView adapterview,
								...
								System.out.println(textInvoiceContent.getText()
										.toString());
							}
						});
	...
	public void updataAddress(String s, String s1) {
		...
		RemoteDataHandler.asyncPost2(Constants.URL_UPDATE_ADDRESS, hashmap,
				new net.asiasofti.android.handler.RemoteDataHandler.Callback() {
					...
					public void dataLoaded(ResponseData responsedata) {
						...
						System.out.println((new StringBuilder("json-->"))
								.append(s2).toString());
						UpdateAddress updateaddress = UpdateAddress
								.newInstanceList(s2);
						if (!updateaddress.getAllow_offpay().equals("0")) {
						...
						try {
							...
							while (iterator.hasNext()) {
								try {
									String s3 = jsonobject.getString(iterator
											.next().toString());
									BuyStep1Activity buystep1activity = BuyStep1Activity.this;
